...
class ActivityLog(models.Model):
    """Logs d'activité des autorités pour audit et statistiques"""
    ACTION_TYPE_CHOICES = [
        ('alert_created', 'Alerte créée'),
        ('threshold_changed', 'Seuil modifié'),
        ('crisis_started', 'Crise démarrée'),
        ('crisis_resolved', 'Crise résolue'),
        ('sms_sent', 'SMS envoyé'),
        ('push_sent', 'Notification Push envoyée'),
    ...
    authority = models.ForeignKey(
        ...
        help_text="Autorité qui a effectué l'action"
    )
    ...
    metadata = models.JSONField(
        default=dict,
        blank=True,
        help_text="Données supplémentaires (zone_id, seuil_ancien, seuil_nouveau, etc.)"
    )

    # Pour suivi de qui a créé le log (généralement Django Admin)
    ...
    def __str__(self):
        return f"{self.authority.username} - {self.get_action_type_display()} ({self.timestamp.strftime('%Y-%m-%d %H:%M')})"


class CrisisManagement(models.Model):
    """Gestion des crises par autorité - suivi du travail effectué"""
    STATUS_CHOICES = [
        ('ongoing', 'En cours'),
        ('resolved', 'Résolue'),
        ('archived', 'Archivée'),
    ...
    authority = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='crisis_management',
        help_text="Autorité responsable de la gestion de la crise"
    )
    ...
    zone = models.ForeignKey(
        ...
        blank=True,
        related_name='crisis_management',
        help_text="Zone affectée par la crise"
    )
    ...
    status = models.CharField(
        ...
        default='ongoing',
        help_text="Statut actuel de la crise"
    )
    ...
    hours_worked = models.DecimalField(
        ...
        decimal_places=2,
        default=0,
        help_text="Heures de travail consacrées à cette crise"
    )

    notifications_sent = models.IntegerField(
        default=0,
        help_text="Nombre de SMS et notifications Push envoyés"
    )
    ...
    resolved_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Date de résolution de la crise"
    )

    # Observations
    notes = models.TextField(
        blank=True,
        help_text="Notes et observations sur la gestion de la crise"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['authority', '-crisis_date']),
            models.Index(fields=['status', '-crisis_date']),
            models.Index(fields=['zone', '-crisis_date']),
        ]

    def __str__(self):
        return f"Crise {self.zone.quartier if self.zone else 'N/A'} - {self.authority.username} ({self.get_status_display()})"

...
```
> Note : `confiance` est un **pourcentage (0-100)**, la confiance du modèle dans la classification de risque renvoyée. `probabilite` est la même valeur ramenée sur une échelle 0-1. `horizon_h` est fixé à 24 (le service ne produit pas de classification de risque distincte par horizon, seulement une estimation du niveau d'eau à 12h/24h/72h — non exposée ici pour l'instant).
